...
parser.add_argument('--gold-file', type=str, required=False, default="../scripts/temp_out.jsonl", help="Path to the gold file")
parser.add_argument('--pred-file', type=str, required=False, default="/home/zpq/PythonProject/drug_combo_spert_final1/utils/outputs/final_predictions.jsonl", help="Path to the predictions file")
...
def create_vectors(gold: List[Dict[str, Any]], test: List[Dict[str, Any]], exact_match: bool, any_comb: bool) \
    ...
    for rel1 in gold:
        ...
        for k, rel2 in enumerate(test):
            ...
            if ((spans_intersecting >= 2) and (not exact_match)) or (score == 1):
                ...
                t_out[(rel2["doc_id"][0], str(rel2["drug_idxs"]), get_label(rel2))].append((get_label(rel1), score))
                found = True
                matched.add(k)
    ...
    for k, rel2 in enumerate(test):
        if k not in matched:
            t_out[(rel2["doc_id"][0], str(rel2["drug_idxs"]), get_label(rel2))].append((Label.NO_COMB.value, 0))
    return g_out, t_out
